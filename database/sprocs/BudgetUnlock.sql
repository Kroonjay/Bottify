...
begin
    ...
    if (selected_budget.reserved - amount) < 0 then
        raise exception 'BudgetInsufficientException : Unlock Amount % Larger than Reserved value for Budget ID %', amount, selected_budget.id;
    -- subtracting the amount from the sender's account 
    end if;
    update budget 
